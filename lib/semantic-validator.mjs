...
// Authorizer bases. The gate emits prefixed forms (e.g. "standing:sa_abc",
// "gate:human_H14_exceeded", "human:<chat_id>") — validate the base, allow
// the suffix to carry provenance detail.
const VALID_AUTHORIZER_BASES = new Set([
  'policy', 'human', 'gate', 'timeout', 'manifest', 'standing'
]);
...
function checkRuleOutcomeConsistency(payload, opts = {}) {
  ...
  if (!authzBase || !VALID_AUTHORIZER_BASES.has(authzBase)) {
    return {
      valid: false,
      code: 'INVALID_AUTHORIZER',
      message: `Unknown authorizer "${payload.authorizer}". Valid bases: ${[...VALID_AUTHORIZER_BASES].join(', ')}.`
    };
  }
...
function checkAuthorizerOutcomeCoherence(payload) {
  ...
  const coherent = {
    ...
    'standing': new Set(['allow']),          // standing approvals only auto-allow
  };

  const base = authorizerBase(authorizer);
  const expected = coherent[base];
