...
//
// Generates and verifies Governed Action Receipts — cryptographic proofs
// that a governed action was evaluated by deterministic policy and decided
// by the stated authority. Supports v0 (inline signing) and v1 (envelope
// signing). Anyone with the public key can verify either format.
//
...
// Re-export for callers that import from receipt.mjs
export { canonicalize, validateSemantics };
...
export function createReceipt(params) {
  ...
  return {
    ...
    evidence: {
      ...
      escalation_source: params.escalation_source ?? null
    },
    signature: null,
...
/**
 * Map v0 receipt nested structure to the flat payload shape that
 * validateSemantics expects. All required fields exist in v0 receipts,
 * just nested under governed_action, decision, and evidence.
 */
function receiptV0ToPayload(receipt) {
  return {
    tool: receipt.governed_action?.tool,
    domain: receipt.governed_action?.domain,
    detail_hash: receipt.governed_action?.detail_hash,
    outcome: receipt.decision?.outcome,
    rule: receipt.decision?.rule,
    authorizer: receipt.decision?.authorizer,
    ts: receipt.decision?.timestamp,
    policy_version: receipt.evidence?.policy_version,
    audit_event_id: receipt.evidence?.audit_event_id,
    audit_prev_hash: receipt.evidence?.audit_prev_hash,
    delegation_chain: receipt.evidence?.delegation_chain,
  };
}
...
export function verifyReceipt(receipt, publicKeyPem) {
  ...
  try {
    ...
    if (ok) {
      // Layer 4: Semantic validation (after signature, before trust) — DWP-01
      const payload = receiptV0ToPayload(receipt);
      const semantic = validateSemantics(payload, { skipTemporalCheck: true });
      if (!semantic.valid) {
        const failed = semantic.checks.find(c => !c.valid);
        return { valid: false, reason: `Signature valid but semantically invalid: ${failed.message}` };
      }
      return {
        valid: true,
...
export function createReceiptV1(params) {
  ...
  const payload = {
    ...
    manifest_agent_id: params.manifest_agent_id ?? null,
    manifest_principal: params.manifest_principal ?? null,
    agent_config_hash: params.agent_config_hash ?? null,
    agent_config_source: params.agent_config_source ?? null,
    agent_fingerprint: params.agent_fingerprint ?? null,
    delegation_chain: params.delegation_chain ?? [],
    audit_event_id: params.audit_event_id,
    audit_prev_hash: params.audit_prev_hash,
    escalation_source: params.escalation_source ?? null,
    // Canary v1 Element D — H15/H14 audit fields. Always emitted; meaningful
    // values arrive when Elements A/B (canary behavior) ship. Schema marks
    // these optional so pre-Element-D receipts continue to validate.
    h15_elapsed_seconds: params.h15_elapsed_seconds ?? null,
    h15_floor_seconds: params.h15_floor_seconds ?? null,
    h15_below_floor: params.h15_below_floor ?? null,
    h14_alert_tier: params.h14_alert_tier ?? 0,
    h14_alert_ack_receipt_id: params.h14_alert_ack_receipt_id ?? null
  };
 ...
 * Agent identity fields (agent_config_hash, agent_config_source,
 * agent_fingerprint) are pulled from the event directly — the bash gate
 * and MCP gate both emit them onto audit entries, so the receipt inherits
 * them unless the caller explicitly overrides via opts.
 */
export function createReceiptV1FromEvent(event, opts = {}) {
  return createReceiptV1({
    ...
    manifest_agent_id: opts.manifest_agent_id ?? null,
    manifest_principal: opts.manifest_principal ?? null,
    agent_config_hash: opts.agent_config_hash ?? event.agent_config_hash ?? null,
    agent_config_source: opts.agent_config_source ?? event.agent_config_source ?? null,
    agent_fingerprint: opts.agent_fingerprint ?? event.agent_fingerprint ?? null,
    delegation_chain: opts.delegation_chain ?? [],
    h15_elapsed_seconds: opts.h15_elapsed_seconds ?? event.h15_elapsed_seconds ?? null,
    h15_floor_seconds: opts.h15_floor_seconds ?? event.h15_floor_seconds ?? null,
    h15_below_floor: opts.h15_below_floor ?? event.h15_below_floor ?? null,
    h14_alert_tier: opts.h14_alert_tier ?? event.h14_alert_tier ?? 0,
    h14_alert_ack_receipt_id: opts.h14_alert_ack_receipt_id ?? event.h14_alert_ack_receipt_id ?? null,
    prev_receipt_hash: opts.prev_receipt_hash ?? null
  });
}
...
export function verifyReceiptV1(receipt, publicKeyPem) {
  ...
  try {
    ...
    // Layer 4: Semantic validation (after signature, before trust)
    const semantic = validateSemantics(payload, { skipTemporalCheck: false });
    if (!semantic.valid) {
      const failed = semantic.checks.find(c => !c.valid);
      return {
        valid: false,
        reason: `Signature valid but semantically invalid: ${failed.message}`,
        payload,
    ...
    return {
      valid: true,
      reason: `Verified. Action "${payload.tool}" in domain "${payload.domain}" was ${payload.outcome} by ${payload.authorizer} at ${payload.ts}.`,
      payload,
 ...
 */
export function verifyReceiptAny(receipt, publicKeyPem, opts = {}) {
  const allowV0 = opts.allowV0 === true;
  ...
  // v0: string receipt_version field — off by default at Published v1.0.
  // Callers that still need to verify legacy v0 receipts must pass
  // { allowV0: true } explicitly. Silent dual-path verification is a
  // conformance hazard: a verifier that accepts both without opt-in can
  // accidentally validate a v0 receipt under v1 assumptions and vice-versa.
  if (receipt.receipt_version) {
    if (!allowV0) {
      return {
        valid: false,
        version: 'v0',
        reason: 'v0 receipt rejected. The Published v1.0 reference verifier rejects v0 by default. Pass { allowV0: true } to enable legacy v0 support.'
      };
    }
