...
async function main() {
  try {
    ...
    if (hmacKeyFile && existsSync(hmacKeyFile)) {
      setHmacKey(hmacKeyFile);
    }
...
async function sendTelegram(chatId, transition, result) {
  ...
  const aggregateInfo = result.aggregate
    ? `*Dominant:* ${result.aggregate.dominant} \\| *Diffuse:* ${result.aggregate.diffuse_weighted}`
    : `*Score:* 0`;
