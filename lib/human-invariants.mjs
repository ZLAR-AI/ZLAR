...
function loadState(humanId) {
  ...
  if (!existsSync(file)) {
    const state = { human_id: humanId, date: today, decisions_today: 0, response_times: [], pending: [], last_ask_epoch: 0, last_ask_epoch_ms: 0, canary_tier: 0, canary_trip_count: 0, timing_observations: [], operator_profile_level: 0, trust_lane: 'guarded', clean_run_count: 0, clean_run_started_epoch: 0 };
  ...
  // A clean run earns speed; a broken run restores friction. These fields
  // persist across UTC rollover — a run is a logical sequence of canary
  // outcomes, not a calendar artifact.
  if (!('clean_run_count' in state)) {
    state.clean_run_count = 0;
    migrated = true;
  }
  if (!('clean_run_started_epoch' in state)) {
    state.clean_run_started_epoch = 0;
    migrated = true;
  }
  // v3.3.6 canary cross-session migration: add per-human canary trigger fields.
  // Trigger eligibility (approvals counter, cooldown, pending lock) belongs to
  // the human, not to any one Claude Code session. Pre-v3.3.6 these lived in
  // var/canary/{session_id}.canary.json and reset on every new session, making
  // recovery accidentally harder across short sessions.
  if (!('canary_approvals_since_last' in state)) {
    state.canary_approvals_since_last = 0;
    migrated = true;
  }
  if (!('canary_last_epoch' in state)) {
    state.canary_last_epoch = 0;
    migrated = true;
  }
  if (!('canary_pending_id' in state)) {
    state.canary_pending_id = '';
    migrated = true;
  }
  if (!('canary_pending_session_id' in state)) {
    state.canary_pending_session_id = '';
    migrated = true;
  }
  if (!('canary_pending_started_epoch' in state)) {
    state.canary_pending_started_epoch = 0;
    migrated = true;
  }
  // v3.3.7 Canary Evidence Hardening: add delivery-evidence fields.
  // canary_pending_msg_id is the message_id Telegram returned for the
  // canary's sendMessage call — proves the card was POSTED. NOT proof
  // of human attention. canary_pending_delivered_epoch is wall-clock
  // when the POST was confirmed. canary_pending_artifact_hash is HMAC
  // over the claim payload, binding the .pending routing artifact to
  // this claim so a tampered or substituted artifact is detectable.
  if (!('canary_pending_msg_id' in state)) {
    state.canary_pending_msg_id = '';
    migrated = true;
  }
  if (!('canary_pending_delivered_epoch' in state)) {
    state.canary_pending_delivered_epoch = 0;
    migrated = true;
  }
  if (!('canary_pending_artifact_hash' in state)) {
    state.canary_pending_artifact_hash = '';
    migrated = true;
  }
...
// DEPRECATED in v3.3.4. Internal canary call sites use recordCanaryOutcome.
// Kept exported for external callers that may still depend on it. Semantics
// frozen as written: slow→guarded; guarded→fast only with trust_lane_grant.
...
//
// recordCanaryOutcome(humanId, outcome, threshold = 5, autoEnabled = true)
//   outcome      "passed" | "failed" | "missed"
//
// passed:  clean_run_count++. If autoEnabled and count >= threshold and
...
//          If autoEnabled is false: cap count at threshold (no drift).
...
//
// Returns { action, fromLane, toLane, count }
//   action ∈ "promoted" | "demoted" | "nochange"
export function recordCanaryOutcome(humanId, outcome, threshold = 5, autoEnabled = true) {
  const state = loadState(humanId);
  const fromLane = state.trust_lane || 'guarded';
  const now = Math.floor(Date.now() / 1000);
  let action = 'nochange';
  let toLane = fromLane;

  if (outcome === 'passed') {
    state.clean_run_count = (state.clean_run_count || 0) + 1;
    if (state.clean_run_count === 1) {
      state.clean_run_started_epoch = now;
    }
    if (autoEnabled && state.clean_run_count >= threshold) {
      if (fromLane === 'slow') {
        state.trust_lane = 'guarded';
        toLane = 'guarded';
        state.trust_lane_auto_promoted = { from: 'slow', to: 'guarded', clean_run_length: state.clean_run_count, ts: now };
        state.clean_run_count = 0;
        state.clean_run_started_epoch = 0;
        action = 'promoted';
      } else if (fromLane === 'guarded') {
        state.trust_lane = 'fast';
        toLane = 'fast';
        state.trust_lane_auto_promoted = { from: 'guarded', to: 'fast', clean_run_length: state.clean_run_count, ts: now };
        state.clean_run_count = 0;
        state.clean_run_started_epoch = 0;
        action = 'promoted';
      } else {
        // fast: count resets, lane unchanged.
        state.clean_run_count = 0;
        state.clean_run_started_epoch = 0;
        action = 'nochange';
      }
    } else if (!autoEnabled && state.clean_run_count > threshold) {
      state.clean_run_count = threshold;
    }
  } else if (outcome === 'failed' || outcome === 'missed') {
    state.clean_run_count = 0;
    state.clean_run_started_epoch = 0;
    if (fromLane === 'fast') {
      state.trust_lane = 'guarded';
    ...
    state.trust_lane_demotion = { reason: `canary_${outcome}`, ts: now, clean_run_reset: true };
  }

  saveState(humanId, state);
  return { action, fromLane, toLane, count: state.clean_run_count };
}

// ─── v3.3.6: Cross-Session Canary Trigger Lifecycle ──────────────────────────
// Trigger eligibility (counter, cooldown, pending lock) is per-human. The
// .pending file in var/canary/{session_id}.canary.pending stays as a routing
// artifact for the existing inbox handler; the authoritative state is here.
// Mirrors lib/human-invariants.sh hi_canary_* helpers.

// Atomic increment of canary_approvals_since_last on the per-human state.
// Called from the gate's human-approved-ask path.
export function recordCanaryApproval(humanId) {
  const state = loadState(humanId);
  state.canary_approvals_since_last = (state.canary_approvals_since_last || 0) + 1;
  saveState(humanId, state);
}

// Returns true iff a canary may fire for this human:
//   counter ≥ minApprovals, cooldown elapsed, no pending canary outstanding.
// Probability gate is the caller's responsibility — keeps this pure.
export function canaryShouldTrigger(humanId, opts = {}) {
  const minApprovals = opts.minApprovals ?? 5;
  const cooldownS    = opts.cooldownS    ?? 300;
  const state = loadState(humanId);

  // Per-human pending lock — one canary outstanding at a time.
  if (state.canary_pending_id) return false;
  if ((state.canary_approvals_since_last || 0) < minApprovals) return false;

  const now = Math.floor(Date.now() / 1000);
  const elapsed = now - (state.canary_last_epoch || 0);
  if (elapsed < cooldownS) return false;
...
//
// fs.openSync(lockPath, 'wx') is the Node equivalent of bash's mkdir-mutex:
// fails with EEXIST if the path already exists, atomically creates it
// otherwise. Loser spins up to LOCK_TIMEOUT_MS, then fails. Holder unlinks
// the lockfile on release.
//
// Design rule: this is the only entry point. Direct acquire/release is
// not exported, so callers cannot leak a held lock if they always go
// through canaryWithLock.

const LOCK_TIMEOUT_MS = 5000;
const LOCK_SPIN_MS = 100;

function canaryLockPath(humanId) {
  return join(STATE_DIR, `${humanId}.lock`);
}

function sleepBusy(ms) {
  // Avoid pulling in 'timers/promises' for a single 100ms sleep in a
  // synchronous critical section. This is intentionally a tight spin.
  const end = Date.now() + ms;
  while (Date.now() < end) { /* spin */ }
}

function canaryWithLock(humanId, body) {
  ensureStateDir();
  const lockPath = canaryLockPath(humanId);
  ...
  while (true) {
    try {
      ...
      if (Date.now() >= deadline) {
        const err = new Error(`canary lock timeout for ${humanId}`);
        err.code = 'ELOCK';
        throw err;
      }
      sleepBusy(LOCK_SPIN_MS);
    }
  }
  try {
    return body();
...
//
// Lifecycle mirrors lib/human-invariants.sh hi_canary_claim_pending /
// hi_canary_record_delivery / hi_canary_release_pending /
// hi_canary_clear_pending. msg_id is delivery evidence (Telegram POST
// returned a message_id; the card was POSTED to the chat). It is NOT
// proof of human attention or proof the human ignored the card.
// Demotion requires delivery evidence AND timeout AND no valid callback.

// Locked CAS. Returns true on win, false if another session already
// holds the per-human pending claim.
export function canaryClaimPending(humanId, canaryId, sessionId) {
  return canaryWithLock(humanId, () => {
    const state = loadState(humanId);
    if (state.canary_pending_id) return false;
    const now = Math.floor(Date.now() / 1000);
    state.canary_pending_id = canaryId;
    state.canary_pending_session_id = sessionId;
    state.canary_pending_started_epoch = now;
    state.canary_pending_msg_id = '';
    state.canary_pending_delivered_epoch = 0;
    state.canary_pending_artifact_hash = '';
    state.canary_approvals_since_last = 0;
    state.canary_last_epoch = now;
    saveState(humanId, state);
    return true;
  });
}

// Records Telegram delivery evidence under the per-human lock. msgId
// is the message_id returned by sendMessage. artifactHash is HMAC over
// the claim payload — binds the .pending routing artifact to this
// claim so a tampered or substituted artifact is detectable at resolve.
// Returns true on success, false if the claim no longer belongs to
// this canaryId (e.g. resolved during the Telegram call — vanishingly
// rare, but cheap to check).
export function canaryRecordDelivery(humanId, canaryId, msgId, artifactHash) {
  return canaryWithLock(humanId, () => {
    const state = loadState(humanId);
    if (state.canary_pending_id !== canaryId) return false;
    state.canary_pending_msg_id = String(msgId || '');
    state.canary_pending_delivered_epoch = Math.floor(Date.now() / 1000);
    state.canary_pending_artifact_hash = String(artifactHash || '');
    saveState(humanId, state);
    return true;
  });
}

// Rollback path: claim succeeded but Telegram POST failed. Only releases
// if pending_id still matches — never releases someone else's claim.
export function canaryReleasePending(humanId, canaryId) {
  return canaryWithLock(humanId, () => {
    const state = loadState(humanId);
    if (state.canary_pending_id !== canaryId) return false;
    state.canary_pending_id = '';
...
// those represent canary cadence and survive resolution. Idempotent.
// v3.3.7: also clears msg_id, delivered_epoch, artifact_hash.
export function canaryClearPending(humanId) {
  const state = loadState(humanId);
  state.canary_pending_id = '';
  state.canary_pending_session_id = '';
  state.canary_pending_started_epoch = 0;
...
// Returns { canaryId, sessionId, startedEpoch, msgId, deliveredEpoch,
// artifactHash } for the outstanding canary, or null if nothing pending.
// v3.3.7 — fields added so checkCanaryResult can read delivery evidence.
export function getCanaryPending(humanId) {
  const state = loadState(humanId);
  if (!state.canary_pending_id) return null;
  return {
    canaryId:        state.canary_pending_id,
    sessionId:       state.canary_pending_session_id || '',
    startedEpoch:    state.canary_pending_started_epoch || 0,
    msgId:           state.canary_pending_msg_id || '',
    deliveredEpoch:  state.canary_pending_delivered_epoch || 0,
    artifactHash:    state.canary_pending_artifact_hash || '',
  };
}
