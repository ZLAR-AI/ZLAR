...
// Atomic write: tmp + rename prevents corrupt JSON on crash mid-write.
// Matches the pattern used in human-invariants.sh (> file.tmp && mv file.tmp file).
...
export function checkResponseVariance(humanId, config = {}) {
  ...
  // If expired, clear — response_times was wiped when lockout started,
  // so min_sample check will fail and return ok naturally.
  const h14Lockout = state.h14_lockout_until || 0;
  if (h14Lockout > 0) {
    const nowS = Math.floor(Date.now() / 1000);
    if (nowS < h14Lockout) {
      return { ok: false, reason: 'rubber_stamping', detail: `H14 cooldown active (${h14Lockout - nowS}s remaining)` };
    }
    // Cooldown expired — clear flag
    state.h14_lockout_until = 0;
    saveState(humanId, state);
  }
  ...
  if (stddev < stddevFloor) {
    ...
    const lockoutUntil = Math.floor(Date.now() / 1000) + 300; // 5-minute cooldown
    state.response_times = [];
    state.h14_lockout_until = lockoutUntil;
    saveState(humanId, state);
    return {
      ok: false,
      reason: 'rubber_stamping',
      detail: `warn/info response time std_dev ${stddev.toFixed(1)}s over last ${times.length} decisions (floor: ${stddevFloor}s) — 5min cooldown set`
    };
  }
  return { ok: true };
}
