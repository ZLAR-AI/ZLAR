...
function loadState(humanId) {
  ...
  if (!existsSync(file)) {
    const state = { human_id: humanId, date: today, decisions_today: 0, response_times: [], pending: [], last_ask_epoch: 0, last_ask_epoch_ms: 0, canary_tier: 0, canary_trip_count: 0 };
...
export function postResponseCheck(humanId, severity, decision, config = {}, actionHash = '') {
  ...
  const askMs = state.last_ask_epoch_ms || (state.last_ask_epoch || 0) * 1000;
  const elapsedMs = Date.now() - askMs;
  recordDecision(humanId, decision, elapsed, severity, config, elapsedMs);
