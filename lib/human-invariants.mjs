...
// Atomically: set pending fields; reset counter; record last_epoch.
// Single sealed write to keep per-human invariants consistent under races.
export function canarySetPending(humanId, canaryId, sessionId) {
  const state = loadState(humanId);
  const now = Math.floor(Date.now() / 1000);
...
// Returns { canaryId, sessionId, startedEpoch } for the outstanding canary,
