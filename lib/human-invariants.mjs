...
function loadState(humanId) {
  ...
  if (!existsSync(file)) {
    const state = { human_id: humanId, date: today, decisions_today: 0, response_times: [], pending: [], last_ask_epoch: 0, last_ask_epoch_ms: 0, canary_tier: 0, canary_trip_count: 0, timing_observations: [], operator_profile_level: 0 };
  ...
  // Calibrated Operator Trust Graduation. Do NOT reset them here.
  if (state.date !== today) {
    state.date = today;
...
export function recordTimingObservation(humanId, elapsedMs, h17FloorMs, h15FloorMs, effectiveFloorMs, bindingFloor, severity, riskScore, outcome, source, config = {}) {
...
export function postResponseCheck(humanId, severity, decision, config = {}, actionHash = '') {
  ...
  if (decision === 'deny') {
    ...
    const h15FloorS = deliberationFloors[severity] || deliberationFloors.info;
    const h15FloorMs = h15FloorS * 1000;
    const effectiveFloorMs = Math.max(h17FloorMs, h15FloorMs);
    recordTimingObservation(humanId, elapsedMs, h17FloorMs, h15FloorMs, effectiveFloorMs, 'none', severity, riskScore, 'deny_accepted', 'deny', config);
  ...
  if (!auth.ok) {
    recordTimingObservation(humanId, elapsedMs, h17FloorMs, h15FloorMs, effectiveFloorMs, bindingFloor, severity, riskScore, 'rejected_h17', 'approve', config);
    return auth;
  }
  ...
  if (!delib.ok) {
    if (severity === 'critical') {
      recordTimingObservation(humanId, elapsedMs, h17FloorMs, h15FloorMs, effectiveFloorMs, bindingFloor, severity, riskScore, 'rejected_h15', 'approve', config);
      return delib;
    }
  ...
  recordTimingObservation(humanId, elapsedMs, h17FloorMs, h15FloorMs, effectiveFloorMs, bindingFloor, severity, riskScore, 'accepted', 'approve', config);
